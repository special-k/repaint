...
class DBname extends RT.Widget
  ...
  onFieldChanged: (eventObject, field, value)->
    switch field
      when DBNAME then @nameEl.nodeValue = value
...
class QueryCount extends RT.Widget
  ...
  onFieldChanged: (eventObject, field, value)->
    switch field
      when COUNTCLASSNAME then @spanEl.className = value
      when NBQUERIES then @countEl.nodeValue = value
...
class TopQuery extends RT.Widget
  ...
  onFieldChanged: (eventObject, field, value)->
    switch field
      when ELAPSEDCLASSNAME then @elapsedClassName.className = value
      when FORMATELAPSED then @formatElapsedEl.nodeValue = value
      when QUERY then @queryEl.nodeValue = value
